...
class RegStep2: UIViewController,UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate,UITextViewDelegate {
...
//        day_ary.add(day_dict)
   ...
   @objc func doneClick5() {
    ...
    //    let str_serid = dict["service_id"] as! Int
        let str_id_str = "\(dict["service_id"]!)"
        
        
       let str_serid = (str_id_str as NSString).integerValue
    ...
    @IBAction func editservice(dict:NSDictionary)
    {
       ...
       // con_id=dict["service_id"] as! Int
        ...
        let str_con =  "\(dict["service_id"]!)"
        
         con_id   =  (str_con as NSString).integerValue
        ...
        //let added_serid=dict["added_service_id"] as! Int
          ...
          let str_sr =  "\(dict["added_service_id"]!)"
        
        let added_serid = (str_sr as NSString).integerValue
...
//                    }
    ...
    func calculation()
    {
        ...
        SVProgressHUD.show(withStatus: "Finishing Your Profile...")
        loadfinish()
...
//            ]
        ...
        let params: Parameters = [Parameter.user_id: user_id,
                                  Parameter.provider_name: prov_name.text!,
                                  Parameter.description: des.text!,
                                  Parameter.days: str_days!,
                                  Parameter.start: str_starts!,
                                  Parameter.end: str_ends!,
                                  Parameter.recurring_services: str_rucservices!,"distance_provided" : "\(distance.text!)",
            Parameter.latitude: lat1!,
            Parameter.longitude: long1!]
        
        
        
        Alamofire.request(Webservice.add_provider_details, method: .post, parameters: params, encoding: JSONEncoding.default, headers: nil)
            .responseJSON { response in
                //debugPrint(response.data)
